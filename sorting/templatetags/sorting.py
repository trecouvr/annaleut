...
def sortlink(context, link_text, sort_field, visible_name=None):
...
def autosort(parser, token):
    ...
    queryset = tokens[1]
    default_sort_key = None if len(tokens) < 3 else tokens[2]
    return SortedQuerysetNode(queryset, default_sort_key)


class SortedQuerysetNode(template.Node):

    def __init__(self, queryset, default_sort_key):
